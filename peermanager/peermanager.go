...
func (pm *PeerManager) Connected(p peer.ID, initialWantHaves []cid.Cid) {
	log2.Event(context.TODO(), "jimbspeermanconnect", logging.Metadata{
		"peer": p,
	})
	pm.Lock()
	defer pm.Unlock()
	...
	if pq.refcnt == 0 {
		log2.Event(context.TODO(), "jimbspeermanconnectaddwants", logging.Metadata{
			"peer":            p,
			"initialWantsLen": len(initialWantsHaves),
		})
